...
-- Query to retrieve all the bookings made by a specific user
SELECT B.B_ID, S.name AS Service_Name, B.U_ID, U.name AS User_Name
FROM Bookings B
...
SELECT U.U_ID, U.name AS User_Name, SUM(P.amount) AS Total_Amount_Spent
FROM Payments P
...
-- find all flights that have been canceled along with the refund amounts
SELECT C.C_ID, B.B_ID, S.name AS Service_Name, C.Refund_Amount, C.date AS Cancellation_Date
FROM Cancellations C
JOIN Bookings B ON C.B_ID = B.B_ID
JOIN Services S ON B.S_ID = S.S_ID;

-- retrieve the average rating for each flight service
SELECT S.S_ID, S.name AS Service_Name, AVG(R.rating) AS Average_Rating
FROM Reviews R
JOIN Services S ON R.S_ID = S.S_ID
GROUP BY S.S_ID, S.name;

-- list all users who have left a review with their ratings and comments:
SELECT U.U_ID, U.name AS User_Name, S.name AS Service_Name, R.rating, R.comment
FROM Reviews R
JOIN Users U ON R.U_ID = U.U_ID
JOIN Services S ON R.S_ID = S.S_ID;

-- get the details of the last 5 invoices generated:
SELECT I.I_ID, U.name AS User_Name, I.amount, I.date AS Invoice_Date
FROM Invoices I
